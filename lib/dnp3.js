...
function encodeFrame({ control, destination, source, userData = Buffer.alloc(0) }) {
    if (userData.length > 250) throw new RangeError('DNP3 user data must not exceed 250 bytes');
...
function decodeFrame(buffer) {
    if (buffer.length < 10 || buffer[0] !== START_1 || buffer[1] !== START_2) return null;
    const userLength = buffer[2] - 5;
    if (userLength < 0) throw new Error('Invalid DNP3 length');
    const totalLength = 10 + userLength + Math.ceil(userLength / 16) * 2;
    if (buffer.length < totalLength) return null;
    const header = buffer.subarray(0, 8);
    if (crcDnp(header) !== buffer.readUInt16LE(8)) throw new Error('Invalid DNP3 header CRC');
    const chunks = [];
    let wireOffset = 10;
    ...
    while (remaining > 0) {
        ...
        if (crcDnp(chunk) !== buffer.readUInt16LE(wireOffset + length)) throw new Error('Invalid DNP3 data CRC');
        chunks.push(chunk);
        wireOffset += length + 2;
        remaining -= length;
    }
...
const POINT_TYPES = {
    '1': 'binaryInput', '2': 'binaryInput',
    '3': 'doubleBitBinaryInput', '4': 'doubleBitBinaryInput',
    '10': 'binaryOutputStatus', '11': 'binaryOutputStatus',
    '20': 'counter', '21': 'frozenCounter', '22': 'counter', '23': 'frozenCounter',
    '30': 'analogInput', '32': 'analogInput',
    '40': 'analogOutputStatus', '42': 'analogOutputStatus',
};
...
function decodeMeasurement(group, variation, buffer, offset) {
    ...
    if (group === 1 || group === 2 || group === 10 || group === 11) {
        ...
        if (!noFlags) flags = buffer[offset++];
...
function decodeObjects(buffer, offset = 0) {
    ...
    while (offset < buffer.length) {
        if (offset + 3 > buffer.length) throw new Error('Truncated DNP3 object header');
...
function decodeApplication(frame) {
    if (frame.userData.length < 3) throw new Error('Truncated DNP3 application fragment');
    ...
    if (!response) return { transport, control, functionCode, points: [] };
    if (frame.userData.length < 5) throw new Error('Truncated DNP3 response');
...
class MasterConnection extends EventEmitter {
    ...
    connect() {
        return new Promise((resolve, reject) => {
            ...
            const timer = setTimeout(() => socket.destroy(new Error('DNP3 connection timeout')), this.options.timeout);
            socket.once('connect', () => {
                clearTimeout(timer);
                this.emit('connect');
                resolve();
    ...
    directOperate(type, index, value) {
        const sequence = this.sequence++ & 0x0f;
        this.socket.write(encodeRequest({
            source: this.options.localAddress,
            destination: this.options.remoteAddress,
            sequence,
...
function encodePointValue(type, value, flags = 0x01) {
    ...
    buffer[0] = flags;
    if (type === 'analogInput' || type === 'analogOutputStatus') buffer.writeInt32LE(Math.round(Number(value)), 1);
    else buffer.writeUInt32LE(Math.max(0, Math.round(Number(value))) >>> 0, 1);
    return buffer;
}

function encodeStaticPoint(point) {
    const variation = OUTSTATION_VARIATIONS[point.type];
    if (!variation) throw new Error(`Unsupported outstation point type ${point.type}`);
    const header = Buffer.alloc(7);
...
class OutstationServer extends EventEmitter {
    ...
    handleFrame(socket, frame) {
        if (frame.destination !== this.options.localAddress && frame.destination < 0xfff0) return;
        if (frame.userData.length < 3) return;
    ...
    handleCommand(socket, frame, sequence, functionCode) {
        ...
        if (qualifier !== 0x28 || request.readUInt16LE(3) !== 1) return;
        ...
        const application = Buffer.concat([
            Buffer.from([0xc0 | sequence, 0x81, 0, 0]),
            responseObject,
        ]);
        const transport = Buffer.concat([Buffer.from([0xc0 | sequence]), application]);
        socket.write(encodeFrame({control: 0x44, destination: frame.source, source: this.options.localAddress, userData: transport}));
