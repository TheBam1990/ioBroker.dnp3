...
test('frame encoding round-trips payload and addresses', () => {
  const encoded = encodeFrame({
    ...
    destination: 1024,
    source: 1,
    userData: Buffer.from('c0c0013c0106', 'hex'),
  });
...
test('outstation response decodes a binary and analog point', () => {
  ...
  assert.deepEqual(
    message.points.map((point) => [point.type, point.index, point.value]),
    [
      ['binaryInput', 7, true],
      ['analogInput', 9, -42],
    ],
...
test('master and outstation exchange an integrity poll and a control', async () => {
  const outstation = new OutstationServer({
    ...
    remoteAddress: 1,
    points: [
      { type: 'binaryInput', index: 5, value: true },
      { type: 'binaryOutputStatus', index: 6, value: false },
    ],
  });
